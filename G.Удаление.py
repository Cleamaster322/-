...
class SecondForm(QWidget):
    ...
    def add(self):
        ...
        cur = con.cursor()
        self.allGenres = {}
        genrebd =cur.execute("""SELECT * FROM genres""").fetchall()
        # Жанры
        for elem in genrebd:
            self.allGenres[elem[0]] = elem[1]
        ...
        tmpGenre = self.btnGenre.text()
        for i in self.allGenres:
            if tmpGenre == self.allGenres[i]:
                genre = i
            else:
                add_bd = f"""INSERT INTO genres(title) VALUES('{tmpGenre}')"""
                cur.execute(add_bd)
                con.commit()
                genre = tmpGenre
